...
<?php
/******************************************************************************************
	Units Definitions page
	This page is used to view/add/edit units of measurements in the system
*******************************************************************************************/ 
...
{	
	...
	$glb_arr_sort_indicators['ABBREV'] = '';
}
/*
...
function process_form()
{
	...
	if ( $adm_entity_op == opcode_DELETE ) {		
		delete_unit($adm_entity_id);
		$location_url = 'Location: adm_units.php';
		header($location_url);	
		exit;					
	}
}
...
/*
	Function: generate_select_units_sql ( )
	Desc:  
...
function show_records()
{
	...
	$sql = generate_select_units_sql();
	$result = mysql_query($sql) or die(mysql_error());	
	
	$markup = "<table class=\"search_results_container\">					
			   <tr class=\"search_results_header\">
			   <td class=\"small_text\" width=\"3%\"><a href=\"javascript: 
				void open_popup('adm_unit_popup.php?state=".state_NEW."','unitEdit',
				'menubar=no,toolbar=no,scrollbars=no',150,450);\">
				<img src=\"images/b.add.png\" alt=\"Add\" title=\"Add\"/></a></td>
			   <td><strong><a href=\"javascript:doSort(document.frm_units,'NAME', '".$sort_by."')\">Unit Name</a></strong>&nbsp;". $glb_arr_sort_indicators['NAME']."</td>
			   <td><strong><a href=\"javascript:doSort(document.frm_units,'ABBREV', '".$sort_by."')\">Code</a></strong>&nbsp;". $glb_arr_sort_indicators['ABBREV']."</td>
			   </tr>" ;
	if ( mysql_num_rows($result) ) {
		...
		while ($row = mysql_fetch_array($result)) {
			...
			$markup .= "<tr class=\"search_results" .($i%2). "\">
						<td class=\"small_text\">
						<a href=\"javascript:void open_popup('adm_unit_popup.php?unit_id=".$pk ."&state=".state_EDIT. "',
						'unitEdit','menubar=no,toolbar=no,scrollbars=no',150,450);\">
						<img src=\"images/b.edit.png\" alt=\"Edit\" title=\"Edit\"/></a>
						</td>";			
			$markup .= "<td>".mysql_decode($row['NAME'])."</td>";
			$markup .= "<td>".mysql_decode($row['ABBREV'])."</td>";
			$markup .= "</tr>";
			++$i;
		}
...
function generate_form() 
{
	...
	$markup  = '<form action="'. $_SERVER['PHP_SELF'] . '" method="post" name="frm_units">' ;
